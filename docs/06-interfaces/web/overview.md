...
|--------|------|
| **List** | シンプルな一覧（デフォルト） |
| **Board** | ラベル駆動のKanban（ドラッグ&ドロップ対応） |
...
**Note:** Boardはラベル操作用。statusは観測事実で自動更新されるため、ドラッグでは変更しない。
**Note:** ラベルは表示名をそのまま保存し、プロジェクト内で一意に扱う。ラベル定義はプロジェクト設定で管理し、デフォルトセットは削除/編集可能。
...
│                                              [Edit: e] [×] │
├───────────────────────────────────────────────────────────┤
│ Status: ● in_progress (auto)  Priority: [high ▼]          │
│ Type:   [feature ▼]        Complexity: [7/10]             │
│ Labels: [blocked, needs_review] (editable)                │
├───────────────────────────────────────────────────────────┤
│ Description:                                              │
│ ユーザー認証機能を実装する。JWT を使用。                    │
├───────────────────────────────────────────────────────────┤
│ Assignee: ai / coder                                      │
│ Selected Session: #123                                    │
...
│ [UI Editor] [YAML Editor]                    [Save: ⌘S]   │
├───────────────────────────────────────────────────────────┤
│  1 │ # agentmine://agents/coder.yaml (DB-backed YAML)     │
│  2 │ name: coder                                          │
...
│   Worktree Path: [.agentmine/worktrees/          ]                  │
...
│ [UI Editor] [YAML Editor]                    [Save: ⌘S]   │
├───────────────────────────────────────────────────────────┤
│  1 │ # agentmine://settings.yaml (DB-backed YAML)         │
│  2 │ project:                                             │
│  3 │   name: my-project                                   │
...
**Note:** 設定はDBに保存され、YAMLエディタはDB-backedデータを編集する。
...
```
┌─ Edit Prompt: agentmine://prompts/coder.md ───────────────┐
...
├───────────────────────────────────────────────────────────┤
│                                                           │
│ ✗ agentmine://agents/coder.yaml:7:3                      │
│   Expected ':' after property name                        │
│                                                           │
│ ⚠ agentmine://agents/coder.yaml:2:9                      │
│   "gpt-5" is not a valid client                           │
│                                                           │
│ ⚠ agentmine://prompts/coder.md:14:5                      │
│   Unknown template variable: {{task.scope}}               │
│                                                           │
│ ⚠ agentmine://settings.yaml:15:3                         │
...
┌─ Task #3: 認証機能の実装 ─────────────────────────────────┐
│                                                           │
│ Status: ● in_progress (merge pending)                     │
│                                                           │
│ ┌─────────────────────────────────────────────────────┐   │
│ │ Worker Result                                       │   │
│ │                                                     │   │
...
| **P4** | 高度な機能 | ラベル駆動ボード、階層表示、キーボード完全対応 |
