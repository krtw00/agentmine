---
depends_on:
  - ./007-five-layer-architecture.md
tags: [decisions, adr, process-management, webui, cross-platform]
ai_summary: "5層アーキテクチャのプロセス管理をNode.js + Web UIで実現し、OS非依存を達成"
---

# ADR-008: プロセス管理とWeb UI可視化

> Status: Accepted
> 最終更新: 2026-01-29

## コンテキスト

ADR-007で5層アーキテクチャ（Orchestrator/Planner/Supervisor/Worker/Reviewer）を採用した。これらの層は複数のAIプロセスとして実行される。

### 課題

| 課題 | 説明 |
|------|------|
| プロセス管理 | 5つのAIプロセスを誰がどう起動・管理するか |
| 可視化 | 各層の状態をリアルタイムで確認したい |
| OS依存 | tmuxはLinux/macOS/WSL2限定、Windows nativeで動かない |

### 検討した選択肢

| 案 | 仕組み | OS非依存 | 可視化 |
|----|--------|---------|--------|
| tmux | シェルがtmuxでプロセス管理 | ❌ | ターミナル |
| Node.js + Web UI | AgentMineがプロセス管理、Web UIで可視化 | ✅ | ブラウザ |
| Docker内tmux | コンテナ内でtmux | ✅ | ターミナル |

## 決定事項

### Node.js + Web UI方式の採用

AgentMineがNode.jsの`child_process`でAIプロセスを管理し、Web UIでリアルタイム可視化する。

```
┌─────────────────────────────────────────────────┐
│  ブラウザ（Web UI）                              │
│  - 各層の出力をリアルタイム表示                   │
│  - タスク状態の可視化                            │
│  - 入力送信                                     │
└─────────────────────────────────────────────────┘
          ↑ WebSocket
┌─────────────────────────────────────────────────┐
│  AgentMine daemon                               │
│  - プロセス管理（child_process）                 │
│  - stdout/stderr キャプチャ                     │
│  - WebSocket配信                                │
│  - タスク/状態管理（DB）                         │
└─────────────────────────────────────────────────┘
          ↓ spawn
┌─────────────────────────────────────────────────┐
│  AIプロセス群                                    │
│  - Orchestrator（Claude Code）                        │
│  - Planner（Claude Code）                       │
│  - Supervisor（Claude Code）                    │
│  - Worker 1..N（Claude Code）                   │
│  - Reviewer（Claude Code）                      │
└─────────────────────────────────────────────────┘
```

### Web UIレイアウト

```
┌─────────────────────────────────────────────────────────────┐
│  AgentMine - Session: auth-feature                          │
├─────────────────────────────────────────────────────────────┤
│ [Orchestrator] [Planner] [Supervisor] [Workers ▼] [Reviewer]      │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Orchestrator                                            [入力 ▼] │
│  ─────────────────────────────────────────────────────────  │
│  > 認証機能を実装してください                                │
│  < 了解しました。Plannerにタスク分解を依頼します。            │
│  < Planner完了: 3タスクに分解されました                      │
│  < Supervisor: Worker起動中 (1/3)                           │
│  ...                                                        │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│  タスク状況                                                  │
│  #1 認証機能 ████████░░ 2/3 完了                            │
│    ├─ #1-1 DBスキーマ ✅                                    │
│    ├─ #1-2 API実装 🔄 Worker-2                              │
│    └─ #1-3 UI実装 ⏳ blocked by #1-2                        │
└─────────────────────────────────────────────────────────────┘
```

### プロセス管理API

```typescript
// packages/core/src/services/process-manager.ts

interface ProcessManager {
  // セッション管理
  createSession(config: SessionConfig): Promise<Session>
  destroySession(sessionId: string): Promise<void>

  // プロセス管理
  spawnProcess(session: Session, role: Role, command: string[]): Process
  killProcess(processId: string): Promise<void>

  // 入出力
  sendInput(processId: string, text: string): void
  onOutput(processId: string, callback: (data: OutputEvent) => void): void

  // 状態
  getProcessStatus(processId: string): ProcessStatus
  listProcesses(sessionId: string): Process[]
}

interface OutputEvent {
  processId: string
  stream: 'stdout' | 'stderr'
  data: string
  timestamp: Date
}
```

### WebSocket API

```typescript
// クライアント → サーバー
interface ClientMessage {
  type: 'subscribe' | 'unsubscribe' | 'input'
  sessionId?: string
  processId?: string
  text?: string
}

// サーバー → クライアント
interface ServerMessage {
  type: 'output' | 'status' | 'task_update'
  processId?: string
  data: any
  timestamp: Date
}
```

### CLIコマンド

```bash
# セッション開始（daemonも起動）
agentmine session start --name auth-feature

# Web UIを開く
agentmine session open  # ブラウザで http://localhost:3000/sessions/xxx

# セッション一覧
agentmine session list

# 入力送信（CLIからも可能）
agentmine session send shogun "認証機能を実装して"

# セッション終了
agentmine session stop
```

### AI CLI設定

各層で使用するAI CLIは設定で指定可能。

```yaml
# .agentmine/config.yaml
session:
  processes:
    shogun:
      command: ["claude"]
      args: ["--context", ".agentmine/context/shogun.md"]
    planner:
      command: ["claude"]
      args: ["--task", "タスク分解"]
    supervisor:
      command: ["claude"]
      args: ["--task", "監視ループ"]
    worker:
      command: ["claude"]
      args: ["--dangerously-skip-permissions"]
    reviewer:
      command: ["claude"]
      args: ["--task", "コードレビュー"]
```

### OS対応

| OS | 対応方法 |
|----|---------|
| Linux | Node.js child_process（spawn） |
| macOS | Node.js child_process（spawn） |
| Windows | Node.js child_process（spawn） |
| WSL2 | Node.js child_process（spawn） |

Node.jsの`child_process.spawn`はクロスプラットフォームで動作する。

## 結果

### ポジティブな影響

- OS非依存（Windows native対応）
- ブラウザでリッチな可視化
- リモートからもアクセス可能
- AgentMineに統合（単一システム）
- タスク状態と統合表示

### ネガティブな影響

- ターミナル直接操作の喪失（tmuxのような柔軟性）
- WebSocket実装コスト
- ブラウザ必須（ヘッドレス運用では不要だが）

### 将来の拡張

- tmux併用オプション（好みで選択）
- SSH経由のリモートセッション
- モバイル対応UI
- 音声入力対応

## 実装計画

### Phase 1: プロセス管理基盤

- `ProcessManager` サービス実装
- 基本的なspawn/kill/output

### Phase 2: WebSocket配信

- WebSocketサーバー実装
- リアルタイム出力配信

### Phase 3: Web UI

- セッション画面実装
- 各層のタブ表示
- 出力ストリーム表示
- 入力送信

### Phase 4: 統合

- タスク状態との連携
- 各層の自動起動フロー

## 関連ADR

- [ADR-007](./007-five-layer-architecture.md) - 5層アーキテクチャ
- [ADR-005](./005-ai-agnostic-orchestration.md) - AI非依存設計

## 関連ドキュメント

- [Web UI](../06-interfaces/web/overview.md) - Web UI設計
